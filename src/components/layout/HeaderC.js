...
export default class HeaderC extends Component {
	...
	componentDidMount() {
		...
		const headerSpaceBarClassName =
			this.props.color === white ? `${hsbWhite}` : `${hsbBlack}`;
		...
		this.headerOpacityDeterminer = () => {
			...
			if (currentPosition > lastScrollPosition) {
				if (this.state.headerClass !== scrollDownClassName) {
					this.setState({
						headerClass: scrollDownClassName,
					});
				}
			} else {
				if (
					this.headerEle !== null &&
					this.state.headerClass !== scrollUpClassName
				) {
			...
			<>
				...
				>
					<img
						...
						alt="logo-header"
					/>
					<Navigation headerColor={this.props.color} />
				</header>
